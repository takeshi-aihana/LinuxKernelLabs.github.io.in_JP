...
最初の二行の ASM 命令は、現在のタスク（プロセス）のディスクリプタにある ``addr_limit`` メンバを使って EDX レジスタに格納されているポインタの妥当性をチェックし、カーネル空間を指しているポインタが無いことを確認しています。
...
「例外テーブル」（**Exception Table**）の中にはアクセスしたユーザ空間のアドレスごとに一個のエントリがあります。
